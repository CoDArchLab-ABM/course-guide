...
Latin Hypercube Sampling ensures that each parameter is sampled more uniformly across its range. It divides each parameter's range into equal intervals and ensures each interval is sampled exactly once [@chalom_parameter_2015].
...
📌 **Cons:** Requires specialised libraries and might not be ideal for small sample sizes
...
To evaluate each parameter's importance in a simulation model, **Random Forest (RF)** can serve as a feature-importance estimator. This involves training an RF model on simulation results and then analysing each input parameter's impact on the output.

### Step-by-Step Guide to Using Random Forest for Parameter Importance

**1. Generate Simulation Data**
First, generate simulation results by systematically sampling multiple parameters using methods like **Random Sampling, Latin Hypercube Sampling (LHS), or Sobol Sampling**.
...
Once the data is prepared, you can train an RF model using **randomForest** in R.
