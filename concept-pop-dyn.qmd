...
> The increase in the built-up area of an archaeological site, which we assume to be a settlement, is explained by population growth due to migratory influx.

We can express this general idea more schematically as a set of **cases** or **scenarios**. Here we are limited to two:

>↑ immigration → ↑ population → ↑ settlement size  
>↓ immigration → ↓ population → ↓ settlement size

We can also simplify this by sketching a causal diagram, a graph where the nodes are the "things" that change (the *variables*), the arrows mark the direction of the effect or *causality*, and their sign (+ or -) indicates the sense of the effect (positive or negative):
...
>Population (variable) is a function of (*depends on*) immigration (variable).
...
Do you think this satisfactorily describes our explanation? Does it leave out something we implicitly assumed with our first informal explanation? Does it go too far, stating something we did not intend in the first place? Answering these questions pushes us away from informal explanations and into the realm of **logic** and broader **contextual knowledge**.

In our example, we can immediately detect that our variables must be expressed in at least two different units (e.g., $m^{2}$ and individuals). We must add a parameter (a variable that remains constant throughout the process) to convert population into settlement size. We will call it *areaPerInhabitant*:
...
Our causal graph will be at this point considerably different, even when we assume time is implicit to all variables:

![Conceptual model after adding a few parameters](assets/images/conceptual-model-example-2.png)

After a few iterations of this reasoning process, our formal expressions will undoubtedly become more complex. The more variables and parameters an explanatory model includes, the more realistic and rich the scenarios it will allow. However, evidence should then control variables and parameters or, at the very least, define them in a meaningful way.

Remember, while defining a parameter adds complexity, it also marks the point in a branch of thought where modelling *stops*: where something that could certainly be described as complex and dynamic is reduced to a fixed value. 
...
**Variable**: it varies with time. It is part of the object of interest, and its dynamics are interpreted as outputs of the phenomenon (&approx; model).  

**Parameter**: it does not vary with time, but it can vary between instances of the phenomenon (&approx; simulation runs). It is considered an input of the phenomenon (&approx; model).
...
By examining the causal diagram and the equations, we can visualise what aspects our model details or simplifies. In this example, the model is clearly focusing more on **population dynamics** as the primary driver of settlement change, rather than other processes that could mediate population and settlement size (e.g., procurement of materials, construction, labour organisation, social norms of cohabitation). Before adding new elements to the model, it is essential to decide whether this is desirable.

## Expressing conditions as logic bifurcations

More often than not, explanations cannot be formalised solely with causal graphs and algebraic expressions like the ones above. One common case is when we want to represent a process that occurs only if certain conditions are met: a logical bifurcation, or branching.

Imagine, for example, that our migration-driven population model takes into account the combined effect of two factors:
...
The first factor can be simple: a hostile relationship will prevent any migration flow, incoming or outgoing. The corresponding diagram, now expressed as a flowchart, could be:
...
The second factor creates another bifurcation, relevant only if the settlement's neighbourhood is *friendly*. If times are *good*, we assume immigration is triggered because the settlement is attractive to new residents. If times are *bad*, emigration is triggered instead, representing the growing number of inhabitants dissatisfied with local living conditions.

![Conceptual model after adding two logic bifurcations](assets/images/conceptual-model-example-7.png)

The more your formal model is composed of algorithms (discontinuous operations) rather than equations (continuous operations), the more complicated it will be to use causal diagrams and the easier it will be to use flowcharts and other specialised diagrams (e.g. UML). However, when it comes to model development and communication, **ANY diagram is better than NO diagram or conceptual formalism at all**.

## Representing distributed and social mechanisms

If we want a formal model that can account for distributed processes (occurring in parallel through the action of multiple entities) and more complex social mechanisms (i.e. multi-dimensional, non-linear), our conceptual model should move toward an **object**-based and, eventually, **agent**-based framework. We can represent distributed processes in many ways, such as formulating variables as **vectors** and **matrices**if equations remain a viable format, or drawing flowcharts to prescribe the behaviour of entities and their potential interactions.
...
Notice that once the process is conceptualised as distributed, it will be increasingly more challenging to keep the description of the conceptual model in a single formal expression or diagram. In the example above, we simplify the diagram by referring to entire chunks of our model by a single meaningful name (e.g., "reproduce"). These named chunks are the best candidates to implement later as functions: bundles of operations that take inputs and return outputs.

For example, let us define "reproduce" as a decision on whether a household will branch a new one given that a certain probability threshold, called the household fission rate, is met; a household-level parameter replacing the population-level natural growth rate:

![A tentative "reproduce" process](assets/images/conceptual-model-example-9.png)

Remember, specifications can still be quite vague and undefined at this stage. For example, how should we determine whether the household fission rate is sufficiently large? Moreover, observe how, yet again, we rely on the promise of a new function, household fission. Thinking of functions in terms of **modules** can help us expand our conceptual model without getting stuck on details we will only handle once we move to implementation.
