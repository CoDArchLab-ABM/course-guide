# ABM in archaeology {#intro-abm-arch}

ABM has conceptual roots and applications in other fields dating back to the early days of computer science. However, this methodology took its current form and was introduced more broadly into the social sciences only in the 1990s.

ABM has been exceptionally well accepted by archaeologists, given its pre-adaptation to distributed and stochastic processes. Most phenomena of interest to archaeology can be represented with ABM, to archaeologists' satisfaction. Other modelling and simulation approaches (*e.g.*, Dynamic Systems) were, and still are, used but often encounter strong resistance because of their higher abstraction and simplification.  

## A transdisciplinary approach

ABM flexibility is also highly valued by archaeology researchers, given that the discipline is historically and thematically positioned between other disciplines at the overlap of the so-called natural and artificial worlds. An ABM model can handle multiple layers of entities and relationships, allowing it to integrate entire models under the same hood.

Disciplines such as ecology, environmental science, and geography have exploited the same advantage. This approach has driven model development in archaeology for more than two decades. Some authors have called this *trans*disciplinary framework the research of socio-ecological systems (SES), defined in close relation to the broader *complexity science* approach. One of the first public successes of ABM in archaeology, the *Artificial Anasazi* model [@Axtell2002], emerged from this approach as a collaboration among researchers orbiting the [Santa Fe Institute](https://www.santafe.edu/).

Despite its positive influence in pushing the field forward, the SES approach has also limited the diversity of scope and theory used with ABM in archaeology. ABM under SES aligns particularly well, for example, with research questions related to landscape and environmental archaeology and formulated from a processualist perspective. This tutorial is no exception. To the potential "new blood" in this field, I recommend keeping your mind open to all questions and theoretical frameworks, especially those you are already invested in.

ABM in archaeology has been prolific, though it remains a small community. Here, we will only cover a small part of the field, specifically from my perspective. For a broader introduction to the many contributions in this field, I refer you to any of the introductions in the References [particularly @angourakis_simulation_2026; also @daems_agent-based_2026]. For a more practical, in-depth introduction, I recommend the textbook by Romanowska, Wren & Crabtree [-@romanowska_agent-based_2021], which includes many practical exercises in NetLogo, though it uses a programming style and philosophy that differ from those used in this course.

## Domains of application and examples

Here is a non-exhaustive list of examples of simulation and ABM in archaeology organised by topics:

- **Ecological dynamics**
  - *Climate patterns*: seasonality [@angourakis_weather_2022], regional variations, climate change [@bocquet-appel_multi-agent_2014] and adaptation [@coto-sarmientoImpactCooperationClimate2025].  
  - *Soils*: erosion and sediment accumulation [@kabora_how_2020; @robinson_modelling_2018; @ullah_water_2019].
  - *Hydrological dynamics*: water availability, run-off, irrigation [@altaweel_salinization_2012; @altaweel_assessing_2012]. 
  - *Vegetation*: plant domestication [@angourakis_human-plant_2022], crop dynamics [@angourakis_weather_2022; @baum_slash_2016; @boogers_sagascape_2022; @Christiansen2006a; @Joyce2019; @robinson_modelling_2018; @hirst_modelling_2025], deforestation and fires [@boogers_sagascape_2022; @nikulina_hunter-gatherer_2024; @snitker_evaluating_2021; @snitker_identifying_2018].  
  - *Non-human animals*: dynamics of wild populations under human influence [@morrison_agent-based_2017; @elzinga_rapid_2025], domestic animal population dynamics [@gunther_animal_2021], herd behaviour, animal husbandry, transhumance [@gunther_animal_2021; @moritz_artificial_2023] (See also [the Dairy-versus-Transhumance model](https://github.com/Andros-Spica/dairy-vs-transhumance), my own work, still unpublished).  
  
- **Anthropological dynamics**
  - *Artefact production*: operational chain (*chaîne opératóire*), authorship and style, material transformations during manufacturing [@sorensen_fireproxies_2018], use and deposition [@gravel-miguel_agent-based_2018].
  - *Individuals*: pedestrian dynamics [@lake_use_2001], foraging [@brantingham_measuring_2006; @Oestmo2016], metabolism, kinship [@Rogers2013a], health and population dynamics (mating and marriage, reproduction, mortality) [@verhagen_modelling_2019], cognition (memory, rationality and learning) [@mokom_modeling_2015; @premo_cultural_2016; @Sousa2019; @acerbi_individual-based_2020], individual-to-individual interactions, including cooperation and competition [@graham_behaviour_2009; @Sousa2019; @white_subsistence_2013; @coto-sarmientoImpactCooperationClimate2025], exchange [@kimBarterMarketAgentBased2024], and even the modelling of Ancient Greek drama characters [@novokhatko_agent-based_2026].
  - *Groups*: household organization and microeconomics [@Christiansen2006a; @Joyce2019], emergence of alliances and organisations [@Cioffi-Revilla2015a; @white_subsistence_2013], group-to-group interactions [@angourakis_land_2014; @angourakis_food_2015; @Angourakis2017a; @Cioffi-Revilla2015a; @Rogers2013a], group mobility [@Rogers2013a; @Santos2015a], logistics and military tactics [@RubioCampillo2012; @rubio-campillo_development_2014; @verhagen_modelling_2019], prestige, reward and punishment, social learning, cultural transmission and norm emergence [@gower-winter_self-adapting_2022; @mokom_modeling_2015; @premo_cultural_2016; @Drost2018; @acerbi_individual-based_2020]. 
  - *Settlements*: population dynamics [@verhagen_modelling_2019; @crema_simulation_2014], resource exploitation [@boogers_sagascape_2022; @hirst_modelling_2025], trade [@carrignon_tableware_2020; @chliaoutakis_agent-based_2020; @lawall_netlogo_2018; @ortega_towards_2014; @sakahira_generating_2021], migration, macro-economics, urbanisation, cultural evolution [@carrignon_tableware_2020; @gower-winter_self-adapting_2022; @lake_cultural_2012; @mokom_modeling_2015; @sakahira_generating_2021], settlement patterns and land use [@altaweel_evaluating_2015; @chliaoutakis_agent-based_2020; @angourakis_land_2014; @Angourakis2017a; @boogers_sagascape_2022; @Joyce2019; @robinson_modelling_2018; @Rogers2013a; @snitker_identifying_2018; @ullah_water_2019; @hirst_modelling_2025], politogenesis [@Cioffi-Revilla2015a; @Rogers2013a; @turchin_historical_2018], catastrophic collapse or abandonment [@kohler_emergence_2012; @mcanany_leaving_2015].
  - *Regional to global*: cooperation and competition between territorial states [@turchin_historical_2018], trade routes [@chliaoutakis_agent-based_2020; @lawall_netlogo_2018; @ortega_towards_2014], human species dispersal [@callegari_agent-based_2013; @coto-sarmientoImpactCooperationClimate2025] and genetic and cultural diffusions [@bocquet-appel_multi-agent_2014; @kovacevic_simulating_2015; @mokom_modeling_2015].  

- **Site formation and archaeological methods**: distribution of artefacts and structures [@gravel-miguel_agent-based_2018], preservation, strata formation and taphonomy [@davies_modelling_2016], sample bias and strategies [@ullahSamplingMattersWhat2026].

## Focus examples

* "Artificial Anasazi" [@Axtell2002; @Janssen2009]

![Artificial Anasazi model in NetLogo](assets/images/ArtificialAnasazi.PNG)

The Artificial Anasazi model was developed to explore population dynamics in Long House Valley, Arizona. The model represents a population of households, with a simplified food economy based on maize cultivation. By simulating this system, researchers tested the hypothesis that climate change was the main cause of the valley's abandonment.

<a href="https://jasss.soc.surrey.ac.uk/12/4/13.html" target="_blank">
<img src="https://jasss.soc.surrey.ac.uk/12/4/13/figure4.jpg" alt="Artificial Anasazi" style="width: 500px; border: none; box-shadow: none; vertical-align:super">
</a>

* "HOMINIDS" [@Griffith2010]

::: {.grid}
::: {.g-col-6}
![Griffin et al. 2010, Fig. 1](assets/images/griffin-et-al-2010-fig1.PNG)
:::
::: {.g-col-6}
![Griffin et al. 2010, Fig. 6b](assets/images/griffin-et-al-2010-fig6b.PNG)
:::
:::

* "MedLanD" [@Barton2012a]

::: {.grid}
::: {.g-col-6}			
![Barton et al. 2012, Fig. 2](assets/images/Barton-et-al-2012-fig2.PNG)
:::
::: {.g-col-6}
![Barton et al. 2012, Fig. 12](assets/images/Barton-et-al-2012-fig12bc.PNG)
:::
:::

* "HouseholdsWorld" [@Rogers2012b]

::: {.grid}
::: {.g-col-6}			
![Rogers et al. 2012, Fig. 2](assets/images/Rogers-et-al-2012-fig1.PNG)
:::
::: {.g-col-6}
![Rogers et al. 2012, Fig. 12](assets/images/Rogers-et-al-2012-fig5.PNG)
:::
:::

* MayaSim [@Heckbert2013]

::: {.grid}
::: {.g-col-6}			
<img src="https://jasss.soc.surrey.ac.uk/16/4/11/Figure1.png" style="width: 400px;">
:::
::: {.g-col-6}
<img src="https://jasss.soc.surrey.ac.uk/16/4/11/Figure3.png" style="width: 400px;">
:::
:::
						
							
* "Indus Village" [@angourakis_two-rainsindus-village-model_2021; @angourakis_how_2020; @angourakis_weather_2022; @angourakis_weather_2025]

::: {.grid}
::: {.g-col-6}			
<img src="https://pub.mdpi-res.com/quaternary/quaternary-05-00025/article_deploy/html/images/quaternary-05-00025-ag.png?1651888892" style="width: 400px; border: none; box-shadow: none; margin-bottom: 0%;" alt="Angourakis et al. 2022, graphical abstract"/>
:::
::: {.g-col-6}
<img src="https://github.com/Andros-Spica/indus-village-model/blob/master/diagrams/RoadMapSoFar_2022-06.png?raw=true" style="width: 400px; border: none; box-shadow: none; margin-bottom: 0%;" alt="Andros-Spica/diagrams/RoadMapSoFar_2022-06.png"/>

:::
:::
repository: <a href="https://github.com/Andros-Spica/indus-village-model" target="_blank">https://github.com/Andros-Spica/indus-village-model</a>

## Unfinished business: representation and validation

Despite the relative success and proliferation of agent-based modelling in archaeology, an unsolved debate remains over the epistemological nature of simulation models as explanatory.

To be explanatory, models must connect a process to a phenomenon, as defined to the best of our knowledge (**representation**), and to the evidence we raise and select as relevant (**validation**). Both archaeologists and non-simulation modellers sometimes dismiss agent-based models in archaeology as straying too far from archaeological evidence. Why should we dare (or even bother) to simulate past processes that cannot be observed through material remains?

Unfortunately, it is too easy to ignore the difference between descriptive and explicative models and leave archaeological interpretations imposed on descriptive models unchecked by formalisation. When a descriptive mathematical model is used, a good validation result (fit given an evidence set) does not guarantee that the interpretative model represents the process to which it is attributed. More importantly, good validation can be mistaken for good representation.

For example, imagine we, as archaeologists, are convinced that humans could feed on a certain kind of rock at some remote point in the past. When we apply the most robust methods in cartography and inferential statistics to evidence of human presence and findings of such rocks, we might find that a positive correlation confirms our belief. However, the representational link between our interpretation and the reality of the past process is weak. Our belief would hold only if it were impermeable to formalisation and a broader knowledge base. Alternatively, perhaps, we would quickly realise the metabolic constraints of stone digestion.

Modelling structures your thoughts about the world, and mathematical modelling does so with extra discipline. Explanatory and, particularly, simulation modelling, however, is doing that specifically for reconstructing processes, stories, or, in other words, the mechanisms or webs of causality that we presume to be behind the semi-static reality we observe (e.g., archaeological materials).

Non-simulation computational modelling in archaeological research
<img src="https://github.com/Andros-Spica/modelling-simulation-graphs/blob/main/representation-and-validation/representation-and-validation_notSimulation.png?raw=true" alt="types of math models" style="width: 650px; border: none; box-shadow: none; margin-bottom: 0%; background-color: azure;"/>

Simulation modelling in archaeological research
<img src="https://github.com/Andros-Spica/modelling-simulation-graphs/blob/main/representation-and-validation/representation-and-validation.png?raw=true" alt="types of math models" class="fragment" style="width: 650px; border: none; box-shadow: none; margin-bottom: 0%; background-color: azure;"/>
						
We will practice distinguishing between **phenomena**, **evidence**, and **mechanism** when designing our own conceptual model.
