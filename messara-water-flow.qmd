...
To calculate land productivity, we use our GIS data combined with parts of the Land submodel in the [Indus Village model](https://github.com/Andros-Spica/indus-village-model) (Angourakis 2021). In this step, we aim to assign a flow accumulation value to each patch based on its relative elevation within the terrain. This serves as a proxy for the region's hydrology beyond the river data we have.
...
We can improve our results by using the `fill-sink` procedure in the original Land model implementation (see code in the [Indus Village repository](https://github.com/Andros-Spica/indus-village-model/tree/master/03-land-model)), based on Huang & Lee (2015).
