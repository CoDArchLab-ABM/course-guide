...
However, with our current code, there are no margin for inheritance: all variables with shared names between the two breeds (`xcor`, `ycor`, `color`, etc.) were set explicitly during initialisation. Go back to `create-traders-per-settlement`, comment-out the line `set color [color] of base`, and run `setup` again. You can now verify that, even without this line, any trader will have the exact same colour than its base settlement. This small line might be useful though, so let us keep it for now.
