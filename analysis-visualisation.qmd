...
To start, we should load and define all necessary elements in R that will be used throughout our script:
...
In R, there are many ways to have our first look at the data. A useful function from the `tidyverse` family is `glimpse`.
...
* whether every time step was recorded,
...
When we vary parameters systematically, we can compare outcomes across settings.
...
When you record metrics at every time step, trajectories become central.
...
For randomly sampled parameter sets, scatter plots are particularly useful (see chapter on sensitivity analysis). However, they can still offer insight when applied to regularly sampled parameters; unlike boxplots, scatter plots show the true variation of data points.
...
We may also want to detect effects visible only in the full simulation trajectory. For this, we can use line plots as before, but we will be limited in how many runs we can include to keep the plot legible.
