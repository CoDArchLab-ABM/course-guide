...
Version control is a system that tracks and manages file changes over time. It’s widely used in software development, but it also applies to any field where file management is essential, including document editing, research, and project management. At its core, version control provides a historical record of changes. It lets users revert to previous versions, see when and why changes were made, and collaborate without risking overwriting each other’s work.

There are two main types of version control: centralised and distributed. Centralised version control systems, such as Subversion (SVN), store files in a central repository. Users check out files, make changes, and then commit them back to the central repository. While effective, centralised systems can be vulnerable if the central server fails. Distributed version control systems, like Git, address this by letting every user keep a complete copy of the repository on their local machine. This setup enhances collaboration and provides redundancy, as users can work offline and synchronise changes with others when connected.
...
- *Collaboration*: Version control systems make collaboration easier and more efficient by allowing multiple users to simultaneously work on the same project. With distributed systems like Git, you can create branches for different features or tasks and later merge changes seamlessly into the main project. This enables teams to work independently and minimise conflicts.

- *Historical Tracking*: Version control systems keep a detailed history of all changes made to the files. This allows users to see who made changes, when, and why. If an issue arises, you can revert to a previous state without losing data, making debugging easier.

- *Backup and Redundancy*: In distributed systems, each user’s local copy is a backup of the entire project. This redundancy reduces the risk of data loss from server failures or other issues and lets users work offline and sync changes later.

- *Version Management*: Version control systems assign unique identifiers to each change, per commit and file changed, usually called a "Git hash" or “commit hash”. A Git hash is a 40-character hexadecimal string, such as 2d3acf90f35989df8f262dc50beadc4ee3ae1560, derived from the commit's contents, including its parent commit(s), timestamp, and author details [REF]. These identifiers allow users to switch between different versions of the project easily. You can also create branches for experimental features and merge them into the main project once they’re stable, which helps integrate new features more smoothly.

- *Enhanced Workflow*: Many version control systems support automated processes such as Continuous Integration (CI) and Continuous Deployment (CD), which streamline development and testing. These systems can automatically test changes before merging, ensuring higher code quality and reducing the risk of introducing bugs.
...
- **Add**/**Stage**: adds a change in the working directory to the staging area, telling Git to include updates to a particular file in the next commit. However, adding or staging doesn't affect the repository because changes aren't recorded until they are committed (see below).
...
- **Branch**: A separate line of development. The default branch is usually called `main` or `master`. Branches allow you to work on features independently before merging them into the main project [[2](https://www.pluralsight.com/resources/blog/cloud/git-terms-explained)].  A branch is a parallel version of the repository within the same repository structure. By branching, developers can isolate work on different features or fixes without altering the main project files. For example, many projects have a main or master branch for the official release version, while other branches support development or testing. Branches are typically merged into the main branch once they are finalised.
...
2. **Check for Git**
   ...
   - If Git is not installed, you will receive an error message or see that the command is unrecognised. You can download the installer from [git-scm.com](https://git-scm.com) and follow the installation instructions.
...
GitHub is a cloud-based platform that enables developers to store, manage, and collaborate on code repositories. It builds on Git by adding collaborative features like pull requests, issue tracking, and discussions, making it easier for teams to work together on software projects. 

GitHub also hosts open-source projects, allowing anyone to contribute or review code. With integrations for CI/CD, project management tools, and documentation, GitHub is a popular choice for developers worldwide to manage both personal and professional projects.
...
In your browser, make sure your GitHub user profile page is saved in Bookmarks for easy access later. 

:::

### GitHub terminology

Understanding the core vocabulary of GitHub operations can help users make the most of the platform, especially for collaborative or open-source projects. Here are some key concepts to keep in mind:

- **Cloning**: Cloning involves creating a local copy of a repository on your machine. By cloning a repository, users can work offline and change files, then push those changes back to the GitHub repository. This process is essential for local development, allowing users to commit changes and manage their workflow effectively with Git commands.

- **Forking**: Forking creates a personal copy of someone else’s GitHub repository in your account. It lets users experiment with changes without affecting the original repository and is often used to contribute to open-source projects. After forking, developers can freely modify their own versions and submit a pull request to propose these changes to the original repository if they have improvements or fixes to offer.

- **Pull Requests**: A pull request (PR) is a way to propose changes in a repository. After modifying a forked or branched version of a repository, a developer can open a pull request, which initiates a review process. This feature is central to collaboration on GitHub, letting others review, discuss, and approve proposed changes before merging them into the main branch.

- **Commits and Push**: A commit is a snapshot of changes in the repository. Every commit includes a message describing the changes, and each commit builds upon previous ones, creating a history of the repository's development. Pushing is uploading these commits to GitHub from a local repository. After a series of commits on a local branch, a user can push those changes to the corresponding branch on GitHub to keep the remote repository up to date.

- **Gists**: A gist is a simple way to share code snippets or single files. Gists can be public or secret, and they are particularly useful for sharing configuration files or code examples. Users can fork and edit Gists, making them a lightweight tool for collaboration and code sharing.

- **Issues and Discussions**: Issues are GitHub’s built-in tracking system for bugs, tasks, and feature requests. They let users report problems, suggest new features, and engage in project-related conversations. Discussions provide a more open, forum-style setting for broader conversation, enabling users to share ideas, ask questions, and contribute knowledge that might not directly relate to specific code changes.
...
GitHub offers several workflows for managing repositories. Here are three common methods:
...
- Choose an owner (user or organisation), a name and a description for the new fork repository. The default will always be an exact copy of the original repository. Select whether to copy only the main branch. Click "Create fork".  
...
**Making Changes**
Edit files in your favourite text editor or IDE.
...
When you place Markdown files (.md) in a GitHub repository, GitHub automatically renders them in the web interface by default, while you can still view and edit the raw Markdown.

GitHub renders Markdown files in browsers in specific ways. Consult [GitHub Docs](https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax) for knowing more about them.

### How to organise repositories

When structuring your repositories, follow common conventions for organising files in subdirectories. This makes projects more readable and more accessible for others to navigate. Here are some commonly used subdirectories:
...
* `assets`: to hold files and subdirectories with closed content and functionality files.  
  ...
  * `assets/js/`: JavaScript source code enabling interactive functionalities (it would also apply to other programming languages in a similar position). Source code like this might also go in the source code folder, if present.  

These folder structures are conventions and not strict rules. You can adapt or modify them based on your project's needs. 

Several community-based proposals for standards exist, including tools that can help automate creating a new project directory with conventional files. For example, for a typical Data Science project using Python, see [Cookiecutter Data Science](https://github.com/drivendataorg/cookiecutter-data-science).
...
### Conventional files
  ...
  * `README.md`: Provides an overview of the project, including what it does, how to set it up, and how to contribute. A few example sections are:  
    * General description  
...
2. **Authorize GitHub Access**:
   ...
   - You will be redirected to GitHub to authorise Zenodo’s access. Approve the request to complete the connection.
...
This connection lets you generate and manage DOIs for GitHub repositories, improving your project’s citation and research accessibility.
