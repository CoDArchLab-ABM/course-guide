...
Before you start, always keep in mind the general workflow when doing simulation experiments this way:
...
From top to bottom, a few points are worth clarifying:
...
Running a single configuration in BehaviorSpace is useful for:
...
Create a BehaviorSpace experiment with **no varying parameters** and **1 repetition**.
...
BehaviorSpace automatically generates all combinations of parameter values. For example, varying harvest variability across several levels allows us to examine how increasing environmental risk influences population stability, mobility, and collapse.

When working with the Anasazi model, parameter variation is often used to:
...
As a rule, parameter ranges should be meaningful rather than exhaustive.
...
In more advanced uses of the Artificial Anasazi model, BehaviorSpace-specific primitives can be used to track or label simulation runs. For example, run numbers can be stored to distinguish between different repetitions of the same parameter set.

This is useful when:

* linking simulation runs to external visualisations,
* debugging unexpected outcomes,
* combining BehaviorSpace results with spatial or temporal exports.
...
Inspect the output data and identify runs with **extreme outcomes** (e.g. earliest collapse).
...
Some analyses of the Artificial Anasazi model require more detailed outputs than a single CSV file can provide—for example, spatial snapshots of settlement locations or land-use patterns.

Using NetLogo’s file output primitives, it is possible to export:
...
When doing so, filenames should include the run number or parameter values to ensure traceability between files and BehaviorSpace results.
...
Although this goes beyond introductory use of BehaviorSpace, it reflects common practice in archaeological simulation studies and prepares students for more advanced workflows.
