...
We now reach the "spicy" part of PondTrade's development. Until this point, settlements are entirely static, and traders are created only in `setup`, then travel back and forth without affecting settlements.
...
To better explore the effects of this feedback loop, we should start adding plots to the interface. Select Plot in the dropdown menu and add two plot objects, as you have done with buttons, sliders, etc. Configure them as follows:
...
We reach the end of the implementation steps for the first-tier PondTrade model. Given all the changes and extensions we made, we must revisit our conceptual model and update it accordingly.
