...
Remember that the core dynamics of the first-tier conceptual model consisted of two overlapping feedback loops. It could be decomposed as:

- Settlements produce goods, and they decay over time.  
- Traders choose destination✅, load, move✅ and unload goods.  
- Settlements' size increases with goods and decays otherwise.  
- Settlements get more or fewer traders depending on their size.  
...
First, implement the general feedback loop described in the conceptual model, connecting settlement size and trade flow.
...
- Settlement size decays over time, down to a minimum size of 1 (use a parameter: `settlementSizeDecayRate`)  
...
to find-direction ; ego = trader
  ...
  ; find where in the route list the trader is
...
We define a very simple "submodel" in which the economic size units of one settlement produce an equivalent amount of transportable value (`cargoValue`), which traders transfer to another settlement, again as economic size units. However, under this mechanism, we increase the economic size of settlements on every trader trip but never decrease it. To avoid this, we must specify a second mechanism that "decays" economic size over time, up to an arbitrary baseline value (i.e., 1).
...
to go
  ...
  ; the size of settlements decays at a constant rate, up to 1 (minimum)
  ask settlements
  [
...
We must add another parameter to the interface to regulate decay: `settlementSizeDecayRate` (0 to 25, in steps of 0.01; default 5). This is the percentage of sizeLevel subtracted at each simulation step.

:::

## Visualising dynamics

To better explore the effects of this feedback loop, add plots to the interface. Select Plot in the drop-down menu and add two plot objects, as you have done with buttons, sliders, etc. Configure them as follows:
...
## Checking the milestone File (step 8)
...
If you followed any of the different debugging approaches (*e.g.*, inspecting agents, printing messages mid-code), you have detected that traders aren't transporting any cargo value. The problem lies in how we scheduled the calls to `unload-cargo` and `load-cargo`. Notice that we added `load-cargo` to a conditional call `ask (turtle-set tradersInBase tradersInDestination) with [cargoValue > 0]`, which is only relevant to traders *with cargo*, which is impossible for traders to fulfil initially.
...
We split the general positive feedback loop into two, as specified in our concept model. These represent how trade flow affects settlement size through changes in the stock of goods and the number of traders per settlement.
...
The key addition is that now we will have to differentiate, for each settlement, between `currentNumberOfTraders` and `potentialNumberOfTraders`, and update these according `sizeLevel`. To wrap this up, we implement an update procedure specifically for settlements (`update-settlements`).

We also need to manage traders left outside the maximum value for their base settlement. We create the tag `isActivated` to avoid creating and deleting too many traders in the same simulation run (i.e., it might become a problem as `who` numbers grow).
...
to update-settlements
  ...
  [
    ...
    ; the sizeLevel of settlements decays at a constant rate, up to 1 (minimum)
    set sizeLevel max (list 1 (sizeLevel * (1 - (settlementSizeDecayRate / 100)) ) )
    ...
    ; conditions favour the creation of new traders
...
Settlements will now calculate `potentialNumberOfTraders` at every simulation step as a number equal to their size and will be allowed to create/reactivate or deactivate any traders accordingly.
...
to update-settlements
  ...
  [
    ...
    ; production is generated in proportion to sizeLevel, following a constant rate
...
In `get-potential-number-of-traders`, there is a *hidden* magic number worth exploring. We assume economic size translates entirely into an increase in the number of traders.
...
As its name suggests, this trait means: "how much the frequency of trade is prioritised over the quality of the cargo". This is because the more traders a settlement has, the less cargo each can get from `stock`, and each has less time to recover. Therefore, this trait highlights an interesting way different settlements could organise trade.

>To test the effect of `frequencyOverQuality`, create a temporary parameter as a slider in the interface named `frequencyOverQuality_temp`. Then, use it to set up the settlement variable, replacing `random-float 1`. Try different values to see whether and how this value affects the model dynamics. Remember to delete this slider and return the code to the original configuration.
...
This graphical description is still far from the implementation code, as it should be. However, it now adequately reflects the new conceptual changes we introduced in the code. 
