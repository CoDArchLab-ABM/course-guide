...
Beyond minor refactoring, the final implementation steps involve displaying (step 12) and recording (step 13) output statistics. These are aggregate measurements of the simulation state at a given time step.

## Displaying outputs in the interface 🤔
...
## Checking the milestone File (step 12)

![Pond Trade step 12](assets/screenshots/BlockB_PondTrade_step12_interface-statistics-interface.png)  
*Pond Trade step 12*

## Recording outputs in variables

The statistics serve as indicators for monitoring complex results while manually running simulations in the interface. Most importantly, you can use them more systematically as data you can export for further use in other software, *e.g.*, to perform advanced statistical analyses and more sophisticated graphs.
...
## Checking the milestone File (step 13)

![Pond Trade step 13](assets/screenshots/BlockB_PondTrade_step13_output-statistics-interface.png)  
*Pond Trade step 13*

We could further implement a procedure to export these and more variables at a specific time in the simulation. We will see an example of this in Block C. However, in most cases, we want to use [NetLogo Behavior Space](https://ccl.northwestern.edu/netlogo/docs/behaviorspace.html). Find it in Tools > Behavior Space. This built-in tool lets you configure experiments to run in parallel within a specific parameter space and run a series of commands before, during, or after a simulation. It includes functionality to export simulation data as a CSV file (you will be prompted with a File Explorer window when you press "run" in an experiment).
