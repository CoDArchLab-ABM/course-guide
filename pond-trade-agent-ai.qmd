...
Implementing this will take several development steps (up to step 9). 
...
However, as you may know, there can be a significant difference between a straight-line measurement (Euclidean distance) and a more complex path cost calculation that uses terrain data. As the Pond Trade model eventually did, we will "complicate" our design and make a proper "least-cost route" calculation, using this as a perfect excuse for learning.

But how should we implement a least-cost path algorithm in NetLogo? We follow our modular philosophy and look for an implementation of the [A* (A-star) algorithm in NetLogo User Community Models](http://ccl.northwestern.edu/netlogo/models/community/Astardemo1). 
...
We are then forced to introduce another parameter, `relativePathCostInLand`, which specifies how much movement on land costs relative to movement in water. Add a slider in the interface (from 0 to 100, in steps of 0.01, default value 50).
...
Given the special circumstance of traders arriving and leaving settlements, let us introduce a differential value for `pathCost` in patches with settlements. For this, we should modify `create-coastal-settlements` and introduce the parameter `relativePathCostInPort` (from 0 to 100, in increments of 0.01, default value 10):
...
Try various coordinates to see how the search process works with different contexts.</small>
...
- be called during `setup` after creating the settlements  
- ask each settlement to find the best routes to every other settlement  
- store the path in a new global variable `routes`  
...
4. asking each settlement to find the best routes to every other settlement  
...
Running the code as-is generates the maximum number of routes given the settlements present (*e.g.*, if `numberOfSettlements = 10`, it will be 45, or 9 + 8 + 7 + 6 + 5 + 4 + 3 + 2 + 1). We store each route as a `list` of `patches`, and all routes as a `list` of routes (`routes`). These will be globally accessible by all traders in our model.

Notice that calculating all possible routes takes considerable time, and the number of settlements increases this time exponentially. At this point, we should know how to stop NetLogo while it's running a long command. Go to Tools > Halt. This might not work if the NetLogo interface is unresponsive. Then the only options are to wait, quit, or force-quit NetLogo.

## Decision making 🤔

Now that we've solved the route calculations, we must use them to inform traders' decisions. How would you do that?
...
- The procedure should:
  - calculate an "attractiveness" score for each settlement (excluding the base settlement)
...
to-report get-routes-from-settlement [ aSettlement ] ; accepts a settlement and returns a list of routes
...
To inform traders' decisions, we define a simple calculation of a route's potential gain (the sum of the sizeLevel of the origin and destination) and calculate a benefit-to-cost ratio for each route. Traders will then compare these ratios for all possible routes from their base settlement and choose the one with the highest value.
...
- The procedure should:
  ...
  - move progressively towards the next patch in the route (not teleporting)
...
- How to move progressively from one patch centre to another?
...
We implement this behaviour assuming movement must go through each patch centre in the route, and that traders move at a maximum speed of `1 / [pathCost] of patch-here`, meaning they are delayed in proportion to the local value of `pathCost`. 
...
As specified in our conceptual model, traders choose a destination whenever they are at their base and move toward their current destination.
...
; by adding ';' at the beginning of each line, or
